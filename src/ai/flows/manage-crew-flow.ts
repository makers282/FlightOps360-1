...
const fetchCrewMembersFlow = ai.defineFlow(
  ...
  async () => {
    ...
    console.log('Executing simplified fetchCrewMembersFlow - Firestore Admin Only');
    
    try {
      const crewDocsSnapshot = await db.collection(CREW_MEMBERS_COLLECTION).get();
      const crewList: CrewMember[] = crewDocsSnapshot.docs.map(doc => {
        const data = doc.data();
        return {
          id: doc.id,
          ...data,
      ...
      console.log(`[fetchCrewMembersFlow] Fetched ${crewList.length} crew members directly from Firestore.`);
      return crewList;

    } catch (error) {
      console.error('Error in fetchCrewMembersFlow:', error);
