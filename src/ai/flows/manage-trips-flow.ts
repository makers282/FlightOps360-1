...
// Add new function to update trip status
export async function updateTripStatus(tripId: string, status: string): Promise<Trip> {
  if (!db) {
    console.error("CRITICAL: Firestore admin instance (db) is not initialized in updateTripStatus.");
    throw new Error("Firestore admin instance (db) is not initialized in updateTripStatus.");
  }
  console.log(`[ManageTripsFlow Firestore Admin] Attempting to update status for trip ID: ${tripId} to ${status}`);
  try {
    const tripDocRef = db.collection(TRIPS_COLLECTION).doc(tripId);
    await tripDocRef.update({
      status: status,
      updatedAt: FieldValue.serverTimestamp(),
    });
    console.log(`Updated status for trip ${tripId} to ${status}.`);
