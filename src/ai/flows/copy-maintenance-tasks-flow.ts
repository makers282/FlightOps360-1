...
import type { MaintenanceTask } from './manage-maintenance-tasks-flow';
  ...
  async ({ sourceAircraftId, taskIds, targetAircraftIds }) => {
    ...
    for (const targetId of targetAircraftIds) {
      ...
      for (const sourceTask of tasksToCopy) {
        // Create a new task object, omitting the ID and resetting completion details
        const newTaskForTarget: Omit<MaintenanceTask, 'id'> = {
          ...sourceTask,
          id: '', // ID will be generated by saveMaintenanceTask
          aircraftId: targetId,
        ...
        // 4. Save the new task. saveMaintenanceTask will generate a new ID internally.
        await saveMaintenanceTask(newTaskForTarget as SaveTaskInput);
        totalCopiedTasks++;
      }
    }
    ...
    return {
      ...
      status: `Successfully copied ${tasksToCopy.length} tasks to ${targetAircraftIds.length} aircraft.`,
    };
  }
);
