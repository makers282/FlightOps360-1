 ...
 */
import { z } from 'zod';
import { legTypes } from '@/app/(app)/quotes/new/components/create-quote-form'; // Assuming legTypes is exported or redefine here
...
export const QuoteLegSchema = z.object({
  ...
  // Potentially add estimated mileage, assumed cruise speed etc. from legEstimates if needed
});
export type QuoteLeg = z.infer<typeof QuoteLegSchema>;

// Main schema for a quote document
export const QuoteSchema = z.object({
  id: z.string().describe("Unique identifier for the quote, typically the auto-generated quoteId."),
  ...
  status: z.enum(["Draft", "Sent", "Accepted", "Rejected", "Expired", "Booked"]).default("Draft"),
  createdAt: z.string().describe("ISO string format, server-generated timestamp").optional(), // Optional on input, server will set
  updatedAt: z.string().describe("ISO string format, server-generated timestamp").optional(), // Optional on input, server will set
...
export const SaveQuoteInputSchema = QuoteSchema.omit({ createdAt: true, updatedAt: true, id: true });
...
export const FetchQuoteByIdInputSchema = z.object({
  quoteId: z.string(), // Using the document ID here
});
